...
int main() {
    ...
    while (true) {
        ...
        if (n == sizeof(commands)) {
            ...
            // Construct 14-byte TX buffer

            // We must send 14 bytes to clock out the 14 bytes of IMU data.

            // Bytes 0-4 contain the command. Bytes 5-13 are dummy zeros.
   ...
   // --- C. SPI Transfer (Byte-by-Byte with Delay) ---
            ...
            // Instead of sending 14 bytes in one blast, we send them one by one

            // with a small pause to let the ATmega ISR finish.

            

            struct spi_ioc_transfer tr[14]; // Array of transfers
            ...
            if (ioctl(spi_fd, SPI_IOC_MESSAGE(14), tr) < 0) {
	    ...
	    int16_t raw_rpm   = combine(rx[12], rx[13]); // Extract RPM
            ...
            // Formula: RPM * (2 * PI / 60)
            float rpm_rads = (float)raw_rpm * 0.104719755f;
            // --- E. Dashboard Display ---
            ...
            printf("   GYRO (deg/s)  : Yaw Rate=%6.2f\n", ratez);

	    printf("   SENSORS       : Wheel speed=%6.2f rad/s (RPM= %d)\n", rpm_rads, raw_rpm);
