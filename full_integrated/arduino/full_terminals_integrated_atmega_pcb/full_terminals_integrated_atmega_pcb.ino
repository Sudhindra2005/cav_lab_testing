...
#define LED1_DDR    DDRH
#define LED1_PORT   PORTH
#define LED1_BIT    PH5

// --- Static LED 2 (Currently Pin 27 / PA5) ---
#define LED2_DDR    DDRH
#define LED2_PORT   PORTH
#define LED2_BIT    PH6

// --- Static LED 3 (Currently Pin 28 / PA6) ---
#define LED3_DDR    DDRH
#define LED3_PORT   PORTH
#define LED3_BIT    PH7

// --- Blinking LED (Currently Pin 29 / PA7) ---
#define LED_BLINK_DDR   DDRB
#define LED_BLINK_PORT  PORTB
#define LED_BLINK_PIN   PINB  // Input register needed for fast toggle
#define LED_BLINK_BIT   PB7
...
void setup() {
    ...
    // --- RPM Init (Direct Register Setup for PK2) ---
    // 1. Configure PK2 as Input with Internal Pull-up
    DDRK  &= ~(1 << PK2);  // Clear bit 2 in Data Direction Register K (Set as Input)
    PORTK |=  (1 << PK2);  // Set bit 2 in Port K (Enable Internal Pull-up)
    
    // 2. Enable Pin Change Interrupts
    cli();
    PCICR  |= (1 << PCIE2);    // Enable Pin Change Interrupts for Port K (PCINT2)
    PCMSK2 |= (1 << PCINT18);  // Enable mask for specific Pin PK2 (PCINT18)
    sei();
...
// ==================== 4. RPM SENSOR ISR =========================
// ISR for Port K (A8-A15). PK2 is A10.
ISR(PCINT2_vect) {
    // Check if PK2 is HIGH (Rising Edge Detection)
    // PINK is the Input Register for Port K
    if (PINK & (1 << PK2)) { 
